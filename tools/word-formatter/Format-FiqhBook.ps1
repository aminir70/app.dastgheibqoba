...
# =============================================================================
# دو شکلِ تیتر در این کتاب‌ها دیده می‌شود:
#   الف) واژه + ترتیب/عنوان   →  «الفصل الأوّل» ، «المبحث السادس في ...»
#   ب ) واژه به‌تنهایی        →  «فصل» و عنوانش در خط بعد (کتاب‌های صلاة)
# «ال» ابتدای واژه هم اختیاری است، چون در بعضی فایل‌ها افتاده.
$ORD        = '(ال\S+|في\b|فيه\b|فيما\b|[0-9\u0660-\u0669]+)'
# واژه‌هایی که تنها آمدنشان هم تیتر حساب می‌شود
$SOLO1      = 'مبحث|باب|قسم'
$SOLO2      = 'فصل|خاتمة|ختام|تتمّة|تتمة|تكملة|مقصد|مقام|مطلب'
$SOLO3      = 'فرع|تنبيه|فائدة|تذنيب'
# واژه‌هایی که فقط با ترتیب/عنوان تیتر حساب می‌شوند (تنها آمدنشان مبهم است)
$ANY1       = 'مبحث|باب|قسم|جزء'
$ANY2       = 'فصل|مقصد|مقام|مطلب|بحث|قول|خاتمة|ختام|تتمّة|تتمة|تكملة'
$ANY3       = 'فرع|جهة|أمر|شرط|تنبيه|فائدة|تذنيب'

$RX_H1      = '^(ال)?(' + $SOLO1 + ')\s*$|^(ال)?(' + $ANY1 + ')\s+' + $ORD + '|^(ال)?مقدّ?مة\s*$'
$RX_H2      = '^(ال)?(' + $SOLO2 + ')\s*$|^(ال)?(' + $ANY2 + ')\s+' + $ORD
$RX_H3      = '^(ال)?(' + $SOLO3 + ')\s*$|^(ال)?(' + $ANY3 + ')\s+' + $ORD
...
function Get-ParagraphClasses {
    ...
    for ($i = 0; $i -lt $Lines.Count; $i++) {
        ...
        # صفحه عنوان (هرچه قبل از اولین تیتر بیاید)
        if (-not $seenHead -and $i -lt 14) { $cls[$i] = 'TITLE'; continue }
    ...
    # --- اگر کتاب سطح بالاتر ندارد، تیترها را یک پله بالا می‌بریم ---
    # مثلاً در کتاب‌های صلاة بالاترین سطح «فصل» است و «مبحث» وجود ندارد؛
    # بدون این کار، ساختار سند از سطح ۲ شروع می‌شد.
    $has1 = $false; $has2 = $false
    foreach ($c in $cls) { if ($c -eq 'H1') { $has1 = $true }; if ($c -eq 'H2') { $has2 = $true } }
    if (-not $has1 -and $has2) {
        for ($i = 0; $i -lt $cls.Count; $i++) {
            if     ($cls[$i] -eq 'H2') { $cls[$i] = 'H1' }
            elseif ($cls[$i] -eq 'H3') { $cls[$i] = 'H2' }
        }
    }
